...
/**
 * Water-surface distortion driven by the pointer, rendered with WebGL.
 *
 * Each pointer sample stamps a ripple into an offscreen displacement buffer;
 * the composite pass then reads that buffer and pushes the image around by
 * however much disturbance sits under each pixel. Ripples are drawn as
 * instanced quads with additive blending, so a hundred of them cost one draw
 * call rather than a hundred.
 *
 * Falls still under `prefers-reduced-motion` — the image stays, the water
 * stops.
 */
...
export interface RippleDistortionProps {
  /** Image the ripples distort. Cover-fitted to the container. */
  src: string;
  /** Diameter of each ripple the pointer lays down, in px. */
  brushSize?: number;
  /** How far the image is pushed, as a fraction of its size. */
  strength?: number;
  /** Full turns the push direction sweeps through as a ripple builds. */
  swirl?: number;
  /** Concentric swells inside each ripple. */
  rings?: number;
  /** How many times its own size a ripple grows to before it dies. */
  spread?: number;
  /** How long a ripple survives, in seconds. */
  fade?: number;
  /** Pointer travel between ripples, in px. */
  spacing?: number;
  /** Chromatic split between red and blue as they refract. */
  dispersion?: number;
  /** Sheen riding the shoulders of each ripple. */
  glint?: number;
  /** Colour the disturbed water takes on. */
  tint?: string;
  tintAmount?: number;
  highlightColor?: string;
  /** Drain the colour so the folds read on tone alone. */
  grayscale?: boolean;
...
export default function RippleDistortion({
  ...
  useEffect(() => {
    ...
    const setNewWave = (x: number, y: number, power: number) => {
      const cfg = configRef.current as { spread: number; brushSize: number };
    ...
    const onMove = (event: PointerEvent) => {
      const cfg = configRef.current as { enabled: boolean; trigger: string; spacing: number };
    ...
    const onDown = (event: PointerEvent) => {
      const cfg = configRef.current as { enabled: boolean; trigger: string; clickStrength: number };
    ...
    const loop = (now: number) => {
      ...
      previousTime = now;
      const cfg = configRef.current as { fade: number };
  ...
  // Live-tunable uniforms, so changing a prop does not rebuild the context.
  useEffect(() => {
